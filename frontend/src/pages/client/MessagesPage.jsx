...
export default function MessagesPage() {
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      setFeedback({ type: 'success', text: 'Message envoye.' });
      setForm({ recipient_id: '', sujet: '', contenu: '' });
      setComposing(false);
      refresh();
    } catch (err) {
      setFeedback({ type: 'error', text: err.response?.data?.message || 'Erreur lors de l\'envoi.' });
    }
  }
  ...
  return (
    <div>
      ...
      {composing && (
        <form className="panel" style={{ marginTop: 'var(--space-4)' }} onSubmit={handleSend}>
          ...
          <div className="form-group">
            <label className="form-label">ID du destinataire</label>
            <input className="form-input" required value={form.recipient_id} onChange={(e) => setForm((f) => ({ ...f, recipient_id: e.target.value }))} placeholder="UUID de l'employe/fournisseur" />
            <span className="form-hint">Vous pouvez repondre directement depuis un message recu pour eviter de saisir l'ID manuellement.</span>
          </div>
          <div className="form-group">
      ...
      <div style={{ display: 'flex', gap: 'var(--space-2)', marginTop: 'var(--space-5)', marginBottom: 'var(--space-3)' }}>
        <button className={`btn btn--sm ${tab === 'inbox' ? 'btn--primary' : 'btn--outline'}`} onClick={() => setTab('inbox')}>Reception ({inbox.length})</button>
        <button className={`btn btn--sm ${tab === 'sent' ? 'btn--primary' : 'btn--outline'}`} onClick={() => setTab('sent')}>Envoyes ({sent.length})</button>
      </div>
          ...
          list.map((m) => (
            <div key={m.id} style={{ padding: 'var(--space-3) 0', borderBottom: '1px solid var(--color-ligne)' }}>
              ...
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-encre-soft)', margin: '2px 0' }}>
                {tab === 'inbox' ? `De : ${m.sender_prenom || ''} ${m.sender_nom || ''} (${m.sender_role || ''})` : `A : ${m.recipient_prenom || ''} ${m.recipient_nom || ''}`}
              </p>
